...
class ModelUtils:
    ...
    def masked_mean(tensor: torch.Tensor, masks: torch.Tensor) -> torch.Tensor:
        ...
        """
        return (tensor.permute(*torch.arange(tensor.ndim - 1, -1, -1)) * masks).sum() / torch.clamp(
            (torch.ones_like(tensor.permute(*torch.arange(tensor.ndim - 1, -1, -1))) * masks).float().sum(), min=1.0
        )
