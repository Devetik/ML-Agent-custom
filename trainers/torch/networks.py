    ...
    def update_normalization(self, buffer: AgentBuffer) -> None:
        ...
        for vec_input, enc in zip(obs, self.processors):
            if isinstance(enc, VectorInput):
                vec_input_array = np.array(vec_input)  # Convertissez d'abord en numpy array
                enc.update_normalization(torch.tensor(vec_input_array))  # Puis convertissez en tensor
...
class NetworkBody(nn.Module):
    def __init__(
        ...
        if (
            ...
            print("Ajout d'une couche ConditionalEncoder")#n'y passe pas
            ...
            print("Ajout d'une couche LinearEncoder")
            self._body_endoder = LinearEncoder(
                total_enc_size, network_settings.num_layers, self.h_size
            )

        if self.use_lstm:
            print("Ajout d'une couche LSTM")
            self.lstm = LSTM(self.h_size, self.m_size)
        else:
            print("Ajout d'une couche INCONNUE")
